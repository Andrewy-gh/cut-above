...
describeDb('Appointment Controller - API Integration', () => {
  ...
  describe('POST /api/appointments', () => {
    ...
    it('prevents double-booking same employee at same time', async () => {
      ...
      expect(response.body.detail).toContain('conflicts with existing appointment');
    });
    ...
    it('rejects end time before start time', async () => {
      ...
      expect(response.body.detail).toContain('End time must be after start time');
    });
    ...
    it('rejects booking when client and employee are the same user', async () => {
      const response = await request(app)
        ...
        .send({
          ...
          service: 'Haircut',
          employee: {
            id: (await User.findOne({ where: { email: 'test@test.com' } }))!.id,
            firstName: 'Test',
          },
        })
        .expect(400);
  ...
  describe('GET /api/appointments', () => {
    ...
    it('rejects admin access', async () => {
      await request(app).get('/api/appointments').set('Cookie', adminSessionCookie).expect(403);
    });
  });
