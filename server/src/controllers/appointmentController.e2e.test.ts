...
describe('Appointment Booking - E2E Flow', () => {
  ...
  const getSessionCookie = (loginRes: {
    headers: Record<string, string | string[] | undefined>;
  }) => {
    const cookies = loginRes.headers['set-cookie'];
    const cookieList = Array.isArray(cookies) ? cookies : cookies ? [cookies] : undefined;
  ...
  describe('Complete booking flow', () => {
    it('registers new client, fetches schedules/employees, books appointment, verifies', async () => {
      ...
      const schedulesRes = await request(app).get('/api/schedules').expect(200);
      ...
      const employeesRes = await request(app).get('/api/employees').expect(200);
