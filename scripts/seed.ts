...
let deploymentUrlSource: 'process.env' | '.env.local' | 'client/.env.local' | 'unknown' = 'unknown';
...
if (maxAppointmentsPerEmployeeDay < minAppointmentsPerEmployeeDay) {
  throw new Error(
    'SEED_MAX_APPOINTMENTS_PER_EMPLOYEE_DAY must be >= SEED_MIN_APPOINTMENTS_PER_EMPLOYEE_DAY.'
  );
}
...
const run = async () => {
  ...
  const employeeBookingsByDay = new Map<string, MinuteRange[]>();
  let userUpserts = 0;
  ...
  let skippedUnavailableSlots = 0;

  for (const user of users) {
    const name = `${user.firstName} ${user.lastName}`.trim();
    const password = mode === 'prod' ? (user as SeedUserWithPassword).password : sharedPassword;
  ...
  for (let dayOffset = 0; dayOffset < totalDays; dayOffset += 1) {
    ...
    for (const employee of employees) {
      ...
      let earliestMinute = 9 * 60 + randomBetween(rand, 0, 4) * 15;
      const employeeBookingKey = getEmployeeBookingKey(date, employeeId);
      const existingEmployeeBookings = employeeBookingsByDay.get(employeeBookingKey) ?? [];
      ...
      for (let slotIndex = 0; slotIndex < targetCount; slotIndex += 1) {
        ...
        const selectedSlot = pickSeedSlot(
          availability.slots,
          existingEmployeeBookings,
          earliestMinute,
          rand
        );
        if (!selectedSlot) {
          skippedUnavailableSlots += 1;
          break;
        }

        const selectedClient = pickClientForSeedSlot({
          clients,
          userMap,
          date,
          candidateRange: selectedSlot.range,
          clientBookingsByDay,
          rand,
        });
        ...
        if (!selectedClient) {
          ...
          earliestMinute = selectedSlot.range.startMinute + 15;
          continue;
        }
        ...
        if (result?.inserted || result?.updated) {
          ...
          const bookingKey = getClientBookingKey(date, selectedClient.clientId);
          const existingBookings = clientBookingsByDay.get(bookingKey) ?? [];
          existingBookings.push(selectedSlot.range);
          clientBookingsByDay.set(bookingKey, existingBookings);
          existingEmployeeBookings.push(selectedSlot.range);
          employeeBookingsByDay.set(employeeBookingKey, existingEmployeeBookings);
        }

        const nextGap = [15, 30, 45][Math.floor(rand() * 3)];
        earliestMinute = selectedSlot.range.endMinute + nextGap;
      }
    }
  }
  ...
  console.log('Seed complete:', {
    ...
    appointmentsUpdated: appointmentUpdates,
    appointmentAttempts,
    skippedClientConflicts,
    skippedUnavailableSlots,
